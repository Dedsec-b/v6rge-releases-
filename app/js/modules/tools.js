...
export class ToolManager {
    ...
    initImageGen() {
        ...
        btn.addEventListener('click', async () => {
            ...
            if (!window.isCloudMode && model === '') return alert('Select a model');
    ...
    async initUpscaler() {
        ...
        if (btn) {
            btn.addEventListener('click', async () => {
                ...
                if (!file) return alert('Please upload an image');
                if (!window.isCloudMode && !modelId) return alert('Select a model');
    ...
    async initBGRemover() {
        ...
        if (btn) {
            btn.addEventListener('click', async () => {
                const file = dropArea.fileObj;
                const modelId = modelSelect.value;
                if (!file || (!window.isCloudMode && !modelId)) return alert('Missing file or model');
    ...
    async initVocalSeparator() {
        ...
        const handleFile = (file) => {
            if (!file.type.startsWith('audio/')) return alert('Audio only');
            dropArea.innerHTML = `<p>Selected: ${file.name}</p>`;
            dropArea.fileObj = file;
            btn.disabled = false;
        };
