...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ================================
    // SIDEBAR LOGIC (Simple Toggle)
    // ================================

    // Listener: Global Event Delegation
    document.body.addEventListener('click', (e) => {
        // Collapse Arrow Click (Sidebar Left)
    ...
    if (submitFeedbackBtn) {
        submitFeedbackBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`${API_URL}/send_feedback`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ type, email, description: desc })
                });

                const result = await response.json();

                if (response.ok) {
                    submitFeedbackBtn.textContent = 'Success ✅';
                    // Clear form
                ...
                submitFeedbackBtn.textContent = 'Failed ❌';
    ...
    if (updateModal) {
        // Listen for main process event
        ipcRenderer.on('show-update-popup', (event, version) => {
            console.log("Update available:", version);
            updateVersionSpan.textContent = `v${version}`;
            updateModal.style.display = 'flex';
        });

        // Handle buttons
        btnUpdateConfirm.addEventListener('click', () => {
            btnUpdateConfirm.textContent = 'Downloading...';
            btnUpdateConfirm.disabled = true;
            btnUpdateCancel.style.display = 'none'; // Hide cancel during download
    ...
    // ================================
    // CHAT FUNCTIONALITY - MOVED TO modules/chat.js
    // Legacy code disabled to prevent conflicts
    // ================================
    /*
    ...
    // ... (Typewriter implementation removed) ...
    // ... (addMessage implementation removed) ...
    */
    // ... (Legacy chat implementation removed - see modules/chat.js) ...
    ...
    function handleToolResult(result) {
        ...
        if (result.type === 'image') {
            ...
            contentHtml = `<div class="generated-content"><pre>${result.output || JSON.stringify(result)}</pre></div>`;
        }
    ...
    function showApprovalModal(request) {
        ...
        card.innerHTML = `
            <h3 style="color: ${headerColor}; margin-bottom: 16px;">⚠️ SYSTEM ACCESS REQUEST</h3>
            <p style="color: #ddd; margin-bottom: 8px;">V6rge wants to ${actionText}</p>
            <div style="background: #000; padding: 12px; border-radius: 6px; color: ${headerColor}; margin-bottom: 20px; text-align: left; word-break: break-all;">
                > ${request.command}
            </div>
            <div style="display: flex; gap: 12px; justify-content: center;">
                ...
                <button id="allow-btn" style="background: ${headerColor}; border: none; color: #111; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-weight: bold;">ALLOW ACCESS</button>
            </div>
        `;
        ...
        modal.querySelector('#deny-btn').addEventListener('click', () => {
            modal.remove();
            addMessage('system', '❌ Access Denied by User.');
        });

        modal.querySelector('#allow-btn').addEventListener('click', async () => {
            modal.remove();
            await executeApprovedTool(request);
        });
    }

    async function executeApprovedTool(request) {
        const loadingDiv = addMessage('system', `> Executing: ${request.command}...`);

        try {
            let data;

            // HYBRID LOGIC: If we have local access (Electron), run locally!
            // Even if using Cloud Brain, we want Local Hands.
            if (window.v6rge && window.v6rge.executeLocalTool && ['terminal', 'read_file', 'list_dir', 'write_file'].includes(request.tool)) {
                console.log("[Hybrid] Routing tool to Local Electron IPC");
                result = await window.v6rge.executeLocalTool(request.tool, request.command);
                data = result;
            } else {
                // Fallback: Send to Backend (Local Python or Cloud API)
                const res = await fetch(`${API_URL}/tool/execute`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        tool: request.tool,
            ...
            if (data.status === 'success') {
                ...
                if (displayOutput && displayOutput.length > 800) displayOutput = displayOutput.substring(0, 800) + '... (truncated)';

                // Feed result back to context as SYSTEM role
                chatHistory.push({ role: 'system', content: `Command Output for '${request.command}':\n${data.output}` });

                // Call LLM for follow-up explanation
                const thinkingDiv = addMessage('assistant', 'Analyzing results...');
                try {
                    // Send result back to Chat Brain (Cloud or Local) to explain it
                    const followUpRes = await fetch(`${API_URL}/chat`, {
                        method: 'POST',
                        body: new URLSearchParams({
                            message: "(System: The command finished. Please explain the result to the user.)",
                            model_id: document.getElementById('chat-model-selector').value,
                            history: JSON.stringify(chatHistory)
                        }) // Implicit Content-Type from URLSearchParams
                    });
                    const followUpData = await followUpRes.json();
                    thinkingDiv.remove();
                    await typewriterMessage(followUpData.response);
                    chatHistory.push({ role: 'assistant', content: followUpData.response });

                } catch (err) {
                    thinkingDiv.remove();
                    addMessage('assistant', `Processing complete. Output:\n${displayOutput}`);
                    chatHistory.push({ role: 'assistant', content: `Processing complete. Output:\n${displayOutput}` });
                }
            ...
            loadingDiv.remove();
            addMessage('system', `Execution Failed: ${e.message}`);
        }
    }
    ...
    if (saveGpuBtn) {
        saveGpuBtn.addEventListener('click', () => {
            ...
            updateGPUDisplay(gpuName, gpuVram);
            window.toast?.success('GPU settings saved!');
        });
    }
